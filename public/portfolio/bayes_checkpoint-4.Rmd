...
First, we'll create some indicator variables for the five regions:
```{r}
indicatorNortheast <- rep(0, nrow(bc_na))
indicatorSouth <- rep(0, nrow(bc_na))
indicatorSouthwest <- rep(0, nrow(bc_na))
indicatorWest <- rep(0, nrow(bc_na))

for(i in 1:nrow(bc_na)){
  indicatorNortheast[i] <- if_else(bc_na$Region[i]=="Northeast", 1, 0)
  indicatorSouth[i] <- if_else(bc_na$Region[i] == "South", 1, 0)
  indicatorSouthwest[i] <- if_else(bc_na$Region[i]=="Southwest", 1,0)
  indicatorWest[i] <- if_else(bc_na$Region[i]=="West", 1, 0)
}
```
...
cap_mod_2 <- "model{
  # Model for Y
  for(i in 1:length(y)){
    y[i] ~ dnorm(beta0 + beta1[2]*indicatorNortheast[i] + beta1[3]*indicatorSouth[i] + beta1[4]*indicatorSouthwest[i]+ beta1[5]*indicatorWest[i], tau)
  }
...
cap_jags_2 <- jags.model(textConnection(cap_mod_2),
                         data=list(y=bc_na$WomenPerClinic, indicatorWest=indicatorWest, indicatorNortheast=indicatorNortheast, indicatorSouth=indicatorSouth, indicatorSouthwest=indicatorSouthwest),
